...
try {
   ...
   results.each { sr ->
      ...
      if(nodeName.size() != 0) {
         ...
         if (!objectClassAttr)
            printer "$notFound${createAttributePair('objectClass', objectClass)}"
         else {
...
void printSummary(def duration) {
   ...
   printer (sprintf(summaryFormat, 'Total users', userCounter,
         "${!userCounter? createSummaryNote('user_objectClass', user_objectClass):''}"))
   printer (sprintf(summaryFormat, 'Total groups', groupCounter,
         "${!groupCounter? createSummaryNote('group_objectClass', group_objectClass):''}"))
   printer (sprintf(summaryFormat, 'Total OUs', ouCounter,
         "${!ouCounter? createSummaryNote('organizationalUnit_objectClass', organizationalUnit_objectClass):''}"))
   printer (sprintf("%-15s %,d ns %s", 'Duration', duration, ''))
}

String createSummaryNote(def attributeName, def attributeVal) {
   "Hint: if this is not correct, check your ${createAttributePair(attributeName, attributeVal)} attribute and your filter."
}

String createAttributePair(def attributeName, def attributeVal) {
...
void printOu(def sr, def parent) {
   ...
   def formatted = sprintf (format,
         'OU', getStringAttribute(sr, 'organizationalUnit_name', organizationalUnit_name), '', '', '', guidToString(sr.attributes[guid]), parent)
   printer formatted
}
...
void printUser(def sr, def parent) {
   userCounter++
   def formatted = sprintf (format,
         'User', getStringAttribute(sr, 'user_firstname', user_firstname), getStringAttribute(sr, 'user_lastname', user_lastname),
         getStringAttribute(sr, 'user_username', user_username), getStringAttribute(sr, 'user_mail', user_mail),
...
void printGroup(def sr, def parent) {
   groupCounter++
   def formatted = sprintf (format,
         'Group', getStringAttribute(sr, 'group_name', group_name), '', '', '', guidToString(sr.attributes[guid]), parent)
   printer formatted
...
String getStringAttribute(SearchResult sr, String attributeName, String attributeVal){

   if (!sr.attributes[attributeVal]) return "[$notFound${createAttributePair(attributeName, attributeVal)})]"

   try{
      return sr.attributes[attributeVal].get().toString()
   }catch(Exception e){
      tout.println "Failed to retrieve attribute '$attributeVal' from ${sr.nameInNamespace}", e
   }
}
...
String guidToString(def guidAttribute) {
   if (!guidAttribute) return "[$notFound${createAttributePair('guid', guid)}]"
