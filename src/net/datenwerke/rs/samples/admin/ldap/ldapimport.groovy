...
def terminalService = GLOBALS.getInstance(TerminalService.class)
def userManagerService = GLOBALS.getInstance(UserManagerService.class)
def userPropertiesService = GLOBALS.getInstance(UserPropertiesService.class)

def ldapImporter = new LdapUserImporterServiceImpl(configService, terminalService, userManagerService, userPropertiesService)
ldapImporter.importUsers()


public class LdapUserImporterServiceImpl {
   ...
   private String getStringAttribute(SearchResult sr, String attributeName) throws NamingException{
      try{
         ...
         logger.log(Level.WARNING, "Failed to retrieve attribute '$attributeName' from ${sr.nameInNamespace}", e)
         return null
      }
   }
   ...
   public void importUsers() throws NamingException{
      loadAllAttributes()
      ...
      logger.log(Level.INFO, 'Starting LDAP Import...')
      logger.log(Level.INFO, "Retrieved nodes from LDAP directory: ${nodesInDirectoryByGuid.size()}")
      logger.log(Level.INFO, "Nodes added: ${addedNodes.size()}")
      logger.log(Level.INFO, "Nodes inhibited: ${inhibitedNodes.size()}")
      int overallCount = countImportedNodes(targetNode) - 1
      logger.log(Level.INFO, "Overall nodes in ReportServer: $overallCount")
      ...
      if((overallCount - inhibitedNodes.size()) != nodesInDirectoryByGuid.size())
         ...
         logger.log(Level.INFO, 'LDAP Import done.')
